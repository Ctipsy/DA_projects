...
def draw_bar(results, word, type):
    ...
    datas = sorted(dict(stats).items(), key=lambda x: x[0], reverse=True)  # dict的排序方法
    for each in datas:
        names.append(each[0])
        values.append(each[1])
    if type == 0:
    ...
    if type == 1:
        write_to_csv(word + "_editors_order", datas)  # 按【 日期：数量 】 排序序后的结果写入csv文件
